...
const skillCategories = {
  ...
  database: ["SQL", "MySQL", "PostgreSQL", "MongoDB", "Redis", "GraphQL", "Oracle", "SQLite", "Firebase"],
  devops: ["Docker", "Kubernetes", "AWS", "Azure", "GCP", "CI/CD", "Git", "GitHub Actions", "Jenkins", "Terraform"],
...
export function generateRandomPosition(id: string): Position {
  ...
  if (suffix === "エンジニア" || suffix === "デベロッパー" || suffix === "プログラマー") {
    // 開発系ポジションの場合
    primaryCategory = getRandomElement(["frontend", "backend", "mobile"]) as keyof typeof skillCategories
  } else if (suffix === "アーキテクト" || suffix === "アドミニストレーター") {
    // インフラ系ポジションの場合
    primaryCategory = getRandomElement(["backend", "devops", "database"]) as keyof typeof skillCategories
  } else if (suffix === "アナリスト") {
    // 分析系ポジションの場合
    primaryCategory = getRandomElement(["database", "other"]) as keyof typeof skillCategories
  } else {
    // その他のポジション
    primaryCategory = getRandomElement(Object.keys(skillCategories)) as keyof typeof skillCategories
  }
