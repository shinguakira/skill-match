...
export default function SkillMatchingApp() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      console.log(
        `${mockEngineers.length}人のエンジニアと${mockPositions.length}件のポジションデータを読み込みました`
      );
    };
  ...
  const filteredEngineers = useMemo(() => {
    return engineers.filter((engineer) => {
      ...
      const matchesKeyword =
        !engineerKeyword || engineer.name.toLowerCase().includes(engineerKeyword.toLowerCase());

      // スキル検索
      const matchesSkills =
        engineerSelectedSkills.length === 0 ||
        engineerSelectedSkills.every((selectedSkill) =>
          engineer.skills.some((skill) => skill.name === selectedSkill)
        );
      ...
      <Tabs defaultValue="engineers">
        ...
        <TabsContent value="engineers" className="space-y-6">
          <div className="flex flex-col gap-4">
            ...
            <div className="text-sm text-muted-foreground">
              {filteredEngineers.length} 件のエンジニアが表示されています（全 {engineers.length}{' '}
              件中）
            </div>
          </div>
          ...
          {filteredEngineers.length > 30 && (
            <div className="text-center py-4 border-t">
              <p className="text-sm text-muted-foreground mb-2">
                パフォーマンスのため、最初の30件のみ表示しています
              </p>
              <p className="text-sm text-muted-foreground">
                検索条件を絞り込むと、より多くの結果を表示できます
              </p>
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="positions" className="space-y-6">
          <div className="flex flex-col gap-4">
            ...
            <div className="text-sm text-muted-foreground">
              {filteredPositions.length} 件のポジションが表示されています（全 {positions.length}{' '}
              件中）
            </div>
          </div>
